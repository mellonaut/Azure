# From https://f.hubspotusercontent40.net/hubfs/7938840/hitepaper%20-%20A%20Guide%20to%20Shift%20Away%20from%20Legacy%20Authentication%20Protocols%20in%20Microsoft%20365%20(2).pdf?__hstc=132955729.b651abe07146502173d026cd90423754.1646928379014.1647157391705.1647161051479.4&__hssc=132955729.3.1647161051479&__hsfp=1205817453&hsCtaTracking=3ef2b1df-b813-4ef4-8083-97f0a2e4269e%7Cc023415b-efc4-4ddc-b232-03332325367f
# Connect as global reader to the tenant
...
function Get-DeepClone {
    ...
    $def_auth_policy = $policies | Where-Object {$_.Name -eq $org_config.
    ...
    $exchange_user = $exchange_users | Where-Object {$_.UserPrincipalName -eq $user.
    UserPrincipalName}
    $user_policy = $policies | Where-Object {$_.Name -eq $exchange_user.
    ...
    $user_protocols.smtp.blocked_method = $user_protocols.smtp.blocked_method +
    "; Transport Config"
    }else{
    ...
    $user_protocols[$protocol].blocked_method = $user_protocols[$protocol].
    blocked_method + "; CAS Mailbox"
    }else{
    ...
    $users | Export-Csv -Path .\BasicProtocolsReport.cs